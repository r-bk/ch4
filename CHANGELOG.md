...
- replace the `tera` build dependency with `zyn`. The template under
  `templates/` is now a real Rust module rather than a Jinja2-style text file,
  which makes it editor-friendly and type-checked.
- drop the external `rustfmt` post-processing step in the build script in favor
  of `prettyplease`, which formats the generated code directly.
- upgrade to `rsdns v0.22.0`
...
- replace `structopt` with latest `clap`. `structopt` is unmaintained and
  contains dependencies with security alerts
- upgrade to `sysinfo v0.32.0`
...
- do not required `rustfmt` in the build script. This makes the code formatting
  a best-effort. And allows building `ch4` on toolchains where `rustfmt` isn't
  installed.
- upgrade to `rsdns v0.18.0`
- auto-upgrade dependencies

## [0.13.0] - 2024-01-19

This is a small, but breaking, release that mainly refreshes dependencies and
updates the MSRV.

### Changed

- bump the MSRV to `1.70.0`
- upgrade to `rsdns v0.17.0` and `smol v2`
- upgrade to `windows v0.52.0`
- upgrade to `sysinfo v0.30.5`
- auto-upgrade all other dependencies
